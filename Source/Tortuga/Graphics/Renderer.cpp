    ...
    std::vector<Vulkan::DescriptorLayout::DescriptorLayout> layouts)
{
    ...
    data.Buffer = Vulkan::Buffer::Create(device, data.ImageSizeBytes, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT);

    std::vector<Pixel> pixels(data.Width * data.Height);
    Vulkan::Buffer::SetData(data.Buffer, pixels.data(), data.ImageSizeBytes);
...
void Render(Renderer data)
{
    ...
    std::vector<Pixel> pixels(data.Width * data.Height);
    Vulkan::Buffer::GetData(data.Buffer, pixels.data(), data.ImageSizeBytes);
    for (uint32_t i = 0; i < pixels.size(); i++)
    {
        if (pixels[i].r != 1 || pixels[i].g != 1 || pixels[i].b != 1 || pixels[i].a != 1)
        {
            Console::Fatal("Pixel assertion failed!");
        }
    }
}
...
void BindDescriptorSets(Renderer data, std::vector<Vulkan::DescriptorSets::DescriptorSets> descriptorSets)
{
    ...
    for (uint32_t i = 0; i < descriptorSets.size(); i++)
        Vulkan::DescriptorSets::UpdateDescriptorSets(descriptorSets[i], i, {data.Buffer});
    ...
    Vulkan::Command::Compute(data.Command, data.Image.Width, data.Image.Height, 1);
